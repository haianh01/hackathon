...
export class EscapeStrategy extends BaseStrategy {
  ...
  public handleEmergency(gameState: GameState): BotDecision | null {
    ...
    if (escapeResult) {
      ...
      const targetPixelPos = escapeResult.nextStep;
      const direction = getDirectionToTarget(currentPos, targetPixelPos);

      console.log(
        `🛤️ EMERGENCY PATHFOUND: Found path to safe zone at cell (${escapeResult.targetCell.x}, ${escapeResult.targetCell.y})`
      );
      console.log(
        `   Next step (Pixel): (${targetPixelPos.x}, ${targetPixelPos.y}), direction: ${direction}`
      );
